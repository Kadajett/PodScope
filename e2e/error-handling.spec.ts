...
test.describe("Error Handling and Degraded States", () => {
  ...
  test("should display error banner when Kubernetes API is down", async ({ page }) => {
  ...
  test("should display error banner when Prometheus API is down", async ({ page }) => {
  ...
  test("should show error message in component when data fetch fails", async ({ page }) => {
    ...
    await expect(page.locator("text=/error|failed|unable/i").first()).toBeVisible({
      timeout: 10000,
    });
  });
  ...
  test("should allow retry after error", async ({ page }) => {
    ...
    if ((await retryButton.count()) > 0) {
      ...
      const healthCheckPromise = page
        .waitForResponse((response) => response.url().includes("/api/health"), { timeout: 10000 })
  ...
  test("should show degraded state when optional services are unavailable", async ({ page }) => {
  ...
  test("should handle component-specific errors without breaking dashboard", async ({ page }) => {
    await apiMocker.mockHealthCheck("healthy");
  ...
  test("should allow navigation to setup page when services are down", async ({ page }) => {
