...
test.describe("Edit Mode and Layout Management", () => {
	...
	test("should open component picker and show available components", async ({
		page,
	}) => {
		await page.goto("/");
		await waitHelper.waitForNetworkIdle();
	...
	test("should allow dragging components to reorder layout", async ({
		page,
	}) => {
		await page.goto("/");
		await waitHelper.waitForNetworkIdle();
	...
	test("should show component settings when clicking settings icon", async ({
		page,
	}) => {
		await page.goto("/");
		await waitHelper.waitForNetworkIdle();
		...
		if (await settingsButton.count() > 0) {
			...
			await expect(
				page.locator('[role="dialog"], .dialog, .modal'),
			).toBeVisible({ timeout: 5000 });
		}
	});
	...
	test("should persist layout changes to localStorage", async ({ page }) => {
		...
		// Get initial config
		const initialConfig = await storageHelper.getDashboardConfig();
	...
	test("should allow exporting dashboard configuration", async ({ page }) => {
		...
		if (await settingsButton.count() > 0) {
			...
			const exportButton = page.locator(
				'button:has-text("Export"), [data-testid="export-config"]',
			);
