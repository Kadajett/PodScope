...
test.describe("Dashboard Functionality", () => {
	...
	test("should load dashboard successfully", async ({ page }) => {
		...
		await expect(
			page.locator("body, main, #__next, [id*='root']"),
		).toBeVisible();
	});

	test("should display health banner when services are down", async ({
		page,
	}) => {
	...
	test("should not display health banner when services are healthy", async ({
		page,
	}) => {
		await page.goto("/");
	...
	test("should display data sources status panel", async ({ page }) => {
		...
		const statusPanel = page.locator(
			'text=/data source|status|connection/i',
		).first();
	...
	test("should display component data after loading", async ({ page }) => {
		...
		const contentElements = page.locator(
			"td, li, .metric, .pod, .node, canvas, svg",
		);
