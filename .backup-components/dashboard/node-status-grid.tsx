...
interface NodeTableRowProps {
  node: KubernetesNode;
  showTaints: boolean;
  onNodeClick: (node: KubernetesNode) => void;
}

function NodeTableRow({ node, showTaints, onNodeClick }: NodeTableRowProps) {
  return (
    <TableRow
      key={node.name}
    ...
    >
      ...
      {showTaints && (
        <TableCell>
          <div className="flex gap-1 flex-wrap max-w-xs">
            {node.taints?.slice(0, 2).map((taint, idx) => (
              <Badge key={`${taint.key}-${idx}`} variant="secondary" className="text-xs">
                {taint.key}
              </Badge>
            ))}
          </div>
